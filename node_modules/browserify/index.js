...
function Browserify (files, opts) {
    ...
    // передали start files
    if (typeof files === 'string' || isarray(files) || isStream(files)) {
        // в opts.entries попали files
        opts = xtend(opts, { entries: [].concat(opts.entries || [], files) });
    }
    // передали только options xtend undefined for in undefined - norm
...
Browserify.prototype._createDeps = function (opts) {
    var self = this;
    // просто копирует все поля объекта в новый объект
    var mopts = copy(opts);
